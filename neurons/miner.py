...
def _load_repo_env(path):
    """Fill os.environ from the repo-local .env (identity must never be
    hardcoded: the manifest reads POKER44_* keys from the environment, so
    every entrypoint — not just the pm2 one — must load the same .env)."""
    try:
